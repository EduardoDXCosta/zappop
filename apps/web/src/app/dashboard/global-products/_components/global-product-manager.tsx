...
export function GlobalProductManager() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    if (formImage) {
      ...
      const upData = await api.post<{ url: string }>('/upload', fd);
    ...
    if (editingId) {
      // Update
      const data = await api.put<{ product: GlobalProduct }>(`/global-products/${editingId}`, body);
      if (data.product) {
        setProducts(prev =>
      ...
      // Create
      const data = await api.post<{ product: GlobalProduct }>('/global-products', body);
      if (data.product) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await api.delete(`/global-products/${id}`);
