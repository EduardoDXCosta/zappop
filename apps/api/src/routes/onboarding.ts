...
import { createTenant, getTenantBySlug, replaceTenantHours } from '../db/queries/tenants.js';
...
function parseHours(value: unknown): Array<{ dayOfWeek: number; opensAt: string; closesAt: string }> {
    ...
    return value
        .map((item) => {
            ...
            if (!Number.isInteger(dayOfWeek) || dayOfWeek < 0 || dayOfWeek > 6 || !opensAt || !closesAt) return null;
            return { dayOfWeek, opensAt: `${opensAt}:00`, closesAt: `${closesAt}:00` };
        })
        .filter((item): item is { dayOfWeek: number; opensAt: string; closesAt: string } => item !== null);
}
...
export async function onboardingRoutes(app: FastifyInstance) {
    ...
    app.post('/tenant', async (request, reply) => {
        ...
        if (!slugBase) {
            return reply.status(400).send({ error: 'could not derive slug from name' });
        }

        const hours = parseHours(body.hours);
        if (hours.length === 0) {
            return reply.status(400).send({ error: 'at least one opening day is required' });
        }
        ...
        const tenant = await createTenant({
            ...
            deliveryNeighborhoods: Array.isArray(body.deliveryNeighborhoods)
                ? body.deliveryNeighborhoods.filter((item): item is string => typeof item === 'string' && item.trim().length > 0)
                : [],
            waitingTimeMinutes: parseNumber(body.waitingTimeMinutes, 30),
            acceptsCard: body.acceptsCard === true,
            cardBrands: Array.isArray(body.cardBrands) ? body.cardBrands.filter((item): item is string => typeof item === 'string' && item.trim().length > 0) : [],
            acceptsVoucher: body.acceptsVoucher === true,
            voucherBrands: Array.isArray(body.voucherBrands) ? body.voucherBrands.filter((item): item is string => typeof item === 'string' && item.trim().length > 0) : [],
            issuesNfCpf: body.issuesNfCpf === true,
            pixKey: cleanString(body.pixKey) ?? undefined,
        });

        await replaceTenantHours({ tenantId: tenant.id, hours });

        reply.send({ tenant: { slug: tenant.slug, name: tenant.name } });
    });
}
