...
function mapInstance(value: unknown): EvolutionInstanceSummary | null {
  ...
  // Evolution API v2 usa `name`, versÃµes anteriores usavam `instanceName`.
...
export async function createInstance(input: {
  ...
  try {
    await evolutionAdminFetch<unknown>('/instance/create', {
      ...
      body: JSON.stringify({
        ...
        reject_call: true,
        msg_call: 'No momento nÃ£o atendemos chamadas por voz.',
        groups_ignore: true,
        always_online: true,
        read_messages: true,
        read_status: true,
    ...
    if (err instanceof Error && (err.message.includes('403') || err.message.includes('already in use'))) {
      console.warn(`[Evolution] InstÃ¢ncia '${input.instanceName}' originou erro 403 "Fantasma". Usando fallback para acionar QR Code diretamente sem recriar.`);
    } else {
      throw err;
    }
  }
  ...
  if (!fetched) {
    // Retorno de fallback caso ela continue dando 404 no "Fetch" mas esteja lÃ¡ com status 403 no "Create"
    return {
      instanceName: input.instanceName,
...
export async function deleteInstance(instanceName: string): Promise<void> {
  try {
    ...
    console.warn(`[Evolution] Falha ignorada ao deletar instÃ¢ncia limpa:`, e);
  }
}
