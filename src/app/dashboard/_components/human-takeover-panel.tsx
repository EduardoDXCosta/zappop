...
export function HumanTakeoverPanel({ tenantId }: { tenantId: string }) {
    ...
    const fetchData = useCallback(async () => {
        try {
            const [takeoversRes, convosRes] = await Promise.all([
                fetch('/api/takeover/active'),
                fetch(`/api/takeover/conversations?tenantId=${tenantId}`),
            ]);
            if (takeoversRes.ok) {
                const data = await takeoversRes.json();
    ...
    async function handleSend(sessionId: string) {
        ...
        try {
            const res = await fetch('/api/takeover/send', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
